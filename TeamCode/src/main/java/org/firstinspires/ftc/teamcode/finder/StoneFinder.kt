...
import org.mechdancer.flow.EventLoopDriver
...
class StoneFinder : UniqueRobotComponent<StoneFinder>(), Sensor<DoubleArray> {
    ...
    private val driver: EventLoopDriver by manager.must()
    ...
    private val location = AtomicReference(DoubleArray(3))
    ...
    override val updated: ISource<DoubleArray> = broadcast()

    override fun update(new: DoubleArray) {
        if (!location.getAndSet(new)!!.contentEquals(new))
            updated post new
    }
